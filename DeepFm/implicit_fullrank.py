...
#to run this code just run this line python implicit_fullrank.py
#To apply gridsearch just run this line  python implicit_fullrank.py --grid_search 1
import argparse
import os
import random
import math
...
def evaluate_model(model, train_user_pos: dict, test_df: pd.DataFrame, num_items: int, genre_matrix, agg_matrix, minilm_matrix, k: int = 10, device: str = 'cpu'):
    ...
    for u in users:
        ...
        # MY CODE Full-ranking evaluation For each user, the held-out item is ranked against ALL unseen items
